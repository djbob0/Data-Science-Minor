...
class ConfigLoader(LoadMaster):
    ...
    def print_table(self):
        ...
        for record in self.table:
            score, config = record
            temp_table.append([score, *[config[key] for key in self.column_of_interest]])
        sorted_scores = list(sorted(temp_table, key=lambda x: x[0]))
        print(tabulate(sorted_scores, headers=['score', *self.column_of_interest]))
...
class ConfigCreator(LoadMaster):
    ...
    def column_combinations(self, index): 
        column_combinations = [ (('thorax_r_x_ext', 'thorax_r_y_ax', 'thorax_r_z_lat'), ('humerus_r_y_plane', 'humerus_r_z_ele', 'humerus_r_y_ax'), ('humerus_l_y_plane', 'humerus_l_z_ele', 'humerus_l_y_ax')),
