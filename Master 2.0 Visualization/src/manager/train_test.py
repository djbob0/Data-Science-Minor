...
class TrainTestManager:
    ...
    def create_split(self) -> ((list, list), (list, list)):
        """
        ...
        train, test = self.generate_data(cat_pat), self.generate_data(cat_pat, invert=True)
    ...
    def generate_data(self, mapping: dict, invert=False) -> (list, list):
        """
        ...
        if not invert:
            for cat in mapping.keys():
                for pat in mapping[cat]:
                    try:
                        data.extend(cat.get_patient(pat).processed)
                        indicator.extend([cat.id] * len(cat.get_patient(pat).processed))
                    except ValueError:
            ...
            for cat in mapping.keys():
                for pat in cat:
                    if pat.id in mapping[cat]:
                        continue
                    try:
                        data.extend(pat.processed)
                        indicator.extend([cat.id] * len(pat.processed))
                    except ValueError:
                        print("patient %i does not exist in category %i" % (pat, cat.id))
                        continue
